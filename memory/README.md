...
Agent memory client for the `surrealdb` Python SDK — a client for
[Spectron](https://github.com/surrealdb/spectron), a memory service.

Shipped as its own distribution so its version moves independently of the SDK's,
and imported through the SDK as `surrealdb.memory`.
...
The extra depends on this package with `>=`, so you can hold `surrealdb` at 3.x
and move `surrealdb-memory` across its own major versions.
...
`Memory` uses `requests`. `AsyncMemory` uses `aiohttp`. Same method names
...
The client covers the full memory service end-user API. Headline verbs are methods on
the client; grouped resources live under namespaces (`memory.documents.…`,
`memory.sessions.…`, etc.). Both `Memory` and `AsyncMemory` expose the same
...
Every `MemoryAPIError` carries `status_code`, `message`, `trace_id`, and
the decoded `body`.
...
carries `X-Memory-On-Behalf-Of: <principal>` and the server runs it as that
