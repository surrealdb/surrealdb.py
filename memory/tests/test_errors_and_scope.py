...
from surrealdb_memory._errors import error_for_status, error_from_response
from surrealdb_memory._retry import backoff_schedule, should_retry
from surrealdb_memory._scope import scope_sets
...
def test_error_for_status_mapping(status: int, cls: type[MemoryAPIError]) -> None:
    exc = error_for_status(status, "boom")
    assert isinstance(exc, cls)
    assert isinstance(exc, MemoryServiceError)
...
def test_error_from_response_extracts_dict_message() -> None:
    ...
    assert isinstance(exc, MemoryNotFoundError)
...
def test_error_from_response_falls_back_to_string_body() -> None:
    ...
    assert isinstance(exc, MemoryAPIError)
