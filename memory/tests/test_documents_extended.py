...
@responses.activate
def test_get_delete_document(client: Memory) -> None:
    responses.add(
        responses.GET,
        f"{DOCS}/doc%3A1",
        json={
...
@responses.activate
def test_list_documents_with_filters(client: Memory) -> None:
    responses.add(
        responses.GET,
        DOCS,
...
@responses.activate
def test_query_documents(client: Memory) -> None:
    responses.add(
        responses.POST,
        f"{DOCS}/query",
        json={
...
@responses.activate
def test_fetch_raw_returns_bytes(client: Memory) -> None:
    responses.add(
        responses.GET,
        f"{DOCS}/doc%3A1/raw",
        body=b"%PDF-1.7 ...",
...
@responses.activate
def test_reprocess_and_recompute(client: Memory) -> None:
    responses.add(
        responses.PUT,
        f"{DOCS}/doc%3A1",
        json={
...
@responses.activate
def test_chunks(client: Memory) -> None:
    responses.add(
        responses.GET,
...
@responses.activate
def test_keyword_subnamespace(client: Memory) -> None:
    responses.add(
        responses.GET,
        f"{DOCS}/keywords",
        json={
