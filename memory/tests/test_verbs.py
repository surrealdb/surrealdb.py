...
@pytest.fixture
def client() -> Memory:
    return Memory(context=CONTEXT, endpoint=BASE, api_key=API_KEY)


@responses.activate
def test_remember_posts_to_facts_with_idempotency_key(client: Memory) -> None:
    responses.add(
        responses.POST,
...
@responses.activate
def test_remember_many_posts_to_facts_batch(client: Memory) -> None:
    responses.add(
        responses.POST,
...
@responses.activate
def test_recall_posts_to_query(client: Memory) -> None:
    responses.add(
        responses.POST,
...
@responses.activate
def test_forget_posts_to_forget(client: Memory) -> None:
    responses.add(
        responses.POST,
...
@responses.activate
def test_chat_non_stream_posts_to_chat(client: Memory) -> None:
    responses.add(
        responses.POST,
...
@responses.activate
def test_documents_upload_posts_multipart(client: Memory, tmp_path: Path) -> None:
...
@responses.activate
def test_scopes_serialise_to_dnf_clauses(client: Memory) -> None:
    responses.add(
        responses.POST,
        f"{BASE}/api/v1/{ENC_CONTEXT}/facts",
...
@responses.activate
def test_on_behalf_of_sends_delegation_header(client: Memory) -> None:
    responses.add(
        responses.POST,
        f"{BASE}/api/v1/{ENC_CONTEXT}/facts",
        json={"mode": "infer", "sessionId": "s"},
    ...
    assert write.headers["X-Memory-On-Behalf-Of"] == "principal:agent-7"
    ...
    assert (
        responses.calls[1].request.headers["X-Memory-On-Behalf-Of"]
        == "principal:agent-7"
    )
    client.recall("q")
    assert "X-Memory-On-Behalf-Of" not in responses.calls[2].request.headers


def test_context_quotes_special_chars(client: Memory) -> None:
...
async def test_async_client_constructs() -> None:
    ...
    client = AsyncMemory(context=CONTEXT, endpoint=BASE, api_key=API_KEY)
    try:
        assert client.context_id == CONTEXT
