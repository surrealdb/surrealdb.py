...
from surrealdb_memory._errors import MemoryAPIError, error_from_response
from surrealdb_memory._retry import backoff_schedule, should_retry
from surrealdb_memory._streaming import ChatChunk, iter_sse_async, iter_sse_blocking
...
def _resolve_api_key(api_key: str | None) -> str:
    if api_key is None or api_key == "":
        raise ValueError("Memory API key is required. Pass api_key=...")
    return api_key


def _resolve_endpoint(endpoint: str | None) -> str:
    if endpoint is None or endpoint == "":
        raise ValueError("Memory endpoint is required. Pass endpoint=...")
...
def on_behalf_of_header(on_behalf_of: str | None) -> dict[str, str]:
    """Build the optional `X-Memory-On-Behalf-Of` delegation header.
    ...
    if not on_behalf_of:
        return {}
    return {"X-Memory-On-Behalf-Of": on_behalf_of}
