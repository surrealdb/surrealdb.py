...
class TestWsCborAdapter(TestCase):
    def test_use_pass(self):
        message = RequestMessage(RequestMethod.USE, params=["ns", "db"])
        outcome = message.WS_CBOR_DESCRIPTOR
        self.assertIsInstance(outcome, bytes)

    def test_use_encoding_only(self):
        """
        Test that CBOR encoding works with valid parameter types only (strict validation enforced).
        """
        message = RequestMessage(RequestMethod.USE, params=["ns", "db"])
        outcome = message.WS_CBOR_DESCRIPTOR
        self.assertIsInstance(outcome, bytes)
    ...
    def test_signin_pass_root(self):
        message = RequestMessage(
            RequestMethod.SIGN_IN, params=[{"username": "user", "password": "pass"}]
        )
        outcome = message.WS_CBOR_DESCRIPTOR
        self.assertIsInstance(outcome, bytes)
    ...
    def test_let_pass(self):
        message = RequestMessage(RequestMethod.LET, params=["key", "value"])
        outcome = message.WS_CBOR_DESCRIPTOR
        self.assertIsInstance(outcome, bytes)
    ...
    def test_live_pass(self):
        message = RequestMessage(RequestMethod.LIVE, params=["person"])
        outcome = message.WS_CBOR_DESCRIPTOR
        self.assertIsInstance(outcome, bytes)

    def test_kill_pass(self):
        message = RequestMessage(
            RequestMethod.KILL, params=["0189d6e3-8eac-703a-9a48-d9faa78b44b9"]
        )
        outcome = message.WS_CBOR_DESCRIPTOR
        self.assertIsInstance(outcome, bytes)

    def test_query_pass(self):
        message = RequestMessage(RequestMethod.QUERY, params=["query", {}])
        outcome = message.WS_CBOR_DESCRIPTOR
        self.assertIsInstance(outcome, bytes)

    def test_create_pass_params(self):
        message = RequestMessage(
            RequestMethod.CREATE, params=["person", {"table": "table"}]
        )
        outcome = message.WS_CBOR_DESCRIPTOR
        self.assertIsInstance(outcome, bytes)
    ...
    def test_insert_pass_dict(self):
        message = RequestMessage(
            RequestMethod.INSERT, params=["table", {"key": "value"}]
        )
        outcome = message.WS_CBOR_DESCRIPTOR
        self.assertIsInstance(outcome, bytes)
    ...
    def test_insert_pass_list(self):
        message = RequestMessage(
            ...
            params=["table", [{"key": "value"}, {"key": "value"}]],
        )
        outcome = message.WS_CBOR_DESCRIPTOR
        self.assertIsInstance(outcome, bytes)
    ...
    def test_update_pass(self):
        message = RequestMessage(
            RequestMethod.UPDATE, params=["test", {"table": "table"}]
        )
        outcome = message.WS_CBOR_DESCRIPTOR
        self.assertIsInstance(outcome, bytes)
    ...
    def test_upsert_pass(self):
        message = RequestMessage(
            RequestMethod.UPSERT, params=["test", {"table": "table"}]
        )
        outcome = message.WS_CBOR_DESCRIPTOR
        self.assertIsInstance(outcome, bytes)
    ...
    def test_merge_pass(self):
        message = RequestMessage(
            RequestMethod.MERGE, params=["test", {"table": "table"}]
        )
        outcome = message.WS_CBOR_DESCRIPTOR
        self.assertIsInstance(outcome, bytes)
