...
def setup_data(blocking_ws_connection):
    ...
    blocking_ws_connection.query(
        "CREATE user:1 SET name = 'User 1';"
    )
    blocking_ws_connection.query(
        "CREATE user:2 SET name = 'User 2';"
    )
    yield
    blocking_ws_connection.query("DELETE user;")
    blocking_ws_connection.query("DELETE likes;")
    ...
    def create_task(task_id: int):
        ...
        try:
            ...
            if result["content"] != f"Document {task_id}":
                errors.append(
                    f"Task {task_id}: Got wrong content: {result['content']}"
                )
    ...
    def create_task(task_id: int):
        ...
        try:
            ...
            if result["id"] != expected_id:
                errors.append(
                    f"create {task_id}: Wrong response {result['id']}"
                )
...
def test_sequential_operations_still_work(blocking_ws_connection, setup_data):
    ...
    # Create a document
    doc = blocking_ws_connection.create(
        RecordID("document", 100),
        {"content": "Test"}
    )
    ...
    # Query to verify
    result = blocking_ws_connection.query("SELECT * FROM document WHERE id = document:100;")
    assert len(result) == 1
