...
async def test_merge_string_with_data(
    ...
    record_id = RecordID("user", "tobie")
    first_outcome = await async_http_connection.update("user:tobie").merge(merge_data)
...
async def test_merge_record_id_with_data(
    ...
    record_id = RecordID("user", "tobie")
    outcome = await async_http_connection.update(record_id).merge(merge_data)
    assert outcome["id"] == record_id
...
async def test_merge_table_with_data(
    ...
    table = Table("user")
    record_id = RecordID("user", "tobie")
    outcome = await async_http_connection.update(table).merge(merge_data)
