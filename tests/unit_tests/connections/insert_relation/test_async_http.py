...
async def setup_data(async_http_connection):
...
async def test_insert_relation_record_ids(async_http_connection, setup_data):
    data = [
        {
...
async def test_insert_relation_record_id(async_http_connection, setup_data):
    data = {
        "in": RecordID("user", "tobie"),
        "out": RecordID("likes", 123),
    }
