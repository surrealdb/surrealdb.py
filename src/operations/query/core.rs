...
pub async fn query(
    ...
    for index in 0..num_statements {
        let value: surrealdb::Value = response.take(index).map_err(|x| x.to_string())?;
        output.push(value.into_inner().into_json());
    }
...
pub async fn select(connection: WrappedConnection, resource: String) -> Result<String, String> {
    let response = match resource.parse::<Thing>() {
        Ok(rid) => match rid.id {
            sql::Id::Range(id_range) => connection
                .connection
                .select(Resource::from(rid.tb))
                .range((
            ...
            _ => connection
                .connection
                .select(Resource::RecordId(RecordId::from_inner(rid)))
                .await
                .map_err(|e| e.to_string())?,
        },
        Err(_) => connection
...
mod tests {
    ...
    fn test_query() {
        ...
        let outcome = runtime.block_on(async {
            ...
            query(
                connection.clone(),
                "CREATE user:tobie SET name = 'Tobie';".to_string(),
                None,
            )
            .await
            .unwrap();
            query(
                connection.clone(),
                "CREATE user:jaime SET name = 'Jaime';".to_string(),
                None,
            )
            .await
            .unwrap();

            query(connection, "SELECT * FROM user;".to_string(), None)
                .await
                .unwrap()
        });
    ...
    fn test_select_all_users() {
        ...
        let outcome = runtime.block_on(async {
            ...
            query(
                connection.clone(),
                "CREATE user:1 SET name = 'Tobie';".to_string(),
                None,
            )
            .await
            .unwrap();
            query(
                connection.clone(),
                "CREATE user:2 SET name = 'Jaime';".to_string(),
                None,
            )
            .await
            .unwrap();
            query(
                connection.clone(),
                "CREATE user:3 SET name = 'Dave';".to_string(),
                None,
            )
            .await
            .unwrap();
    ...
    fn test_select_range_users() {
        ...
        let outcome = runtime.block_on(async {
            ...
            query(
                connection.clone(),
                "CREATE user:4 SET name = 'Tom';".to_string(),
                None,
            )
            .await
            .unwrap();
